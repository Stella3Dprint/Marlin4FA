...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  KEEPALIVE_STATE(PAUSED_FOR_USER);
  #if ENABLED(STM_FA400)
    TERN_(HOST_PROMPT_SUPPORT, hostui.prompt_do(PROMPT_USER_CONTINUE, F("Insert filament and press button to continue"), FPSTR(CONTINUE_STR)));
  #else
  ...
  while (wait_for_user) {
    ...
    if (nozzle_timed_out) {
      ...
      #if ENABLED(STM_FA400)
        TERN_(HOST_PROMPT_SUPPORT, hostui.prompt_do(PROMPT_USER_CONTINUE, F("Heater Timeout"), F("Reheat")));
      #else
      ...
      #if ENABLED(STM_FA400)
        TERN_(HOST_PROMPT_SUPPORT, hostui.prompt_do(PROMPT_USER_CONTINUE, F("Reheat Done"), FPSTR(CONTINUE_STR)));
      #else
...
void resume_print(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/, const celsius_t targetTemp/*=0*/ DXC_ARGS) {
  ...
  // STELLAMOVE
  #ifdef HAS_PITTA_MMU
    PITTA::load_material_from_retract_ready();
  #endif    
