...
void pitta_idle_req() {
  static uint32_t prev_time, cur_time;
  cur_time = millis();
...
void mExtruder_force_dir(bool dir) {
  {
    b_stored_dir = dir;
    cur_ext_dir = dir;
    mExtruder_step(LOW);
...
int16_t tb_len = 0, resrv1 = 0, resrv2 = 0, resrv3 = 0, resrv4 = 0, ptrn_n = 5;

bool quick_chk_material() {
  bool b_material = false;
  b_material = READ(ONE_W_CMD_PIN);
  return b_material;
}
...
#define VAL_EXT_EXPIRE 1500 //4000
...
void PITTA::load_material_from_retract_ready() { 
  ...
  //SERIAL_ECHOLNPGM("load_material_from_retract_ready()");
  mExtruder_force_dir(NOM_DIR);
  delay(10);
  idle();

  while (b_material_empty && wait_expire_cnt < 500000) {
    delay_cnt++;
    if (delay_cnt>20) {
      delay_cnt = 0;
      pitta_req_manage_heater_update();
    ...
    delayMicroseconds(100/WAIT_MUL_V);
    pitta_wtcdog_reset();
    wait_expire_cnt++;
  }
...
void PITTA::initial_retract_ready()
{
  bool b_step = false;
  ...
  SERIAL_ECHOLNPGM("retract_ready() : ",PITTA::b_retract_already);
  ...
  while (!b_material_empty && wait_expire_cnt < 130000) {//40000//1800000
    ...
    else if (wait_expire_cnt<180) {
      delayMicroseconds(90/WAIT_MUL_V);
    }
    else if (wait_expire_cnt<600) {
      delayMicroseconds(70/WAIT_MUL_V);
    }
    else if (wait_expire_cnt<1500) {
      delayMicroseconds(800/WAIT_MUL_V);
    }
    else if (wait_expire_cnt<4500) {
      delayMicroseconds(5500/WAIT_MUL_V);
    }
    // else if (wait_expire_cnt<5000) {
    //   delayMicroseconds(2000/WAIT_MUL_V);
    // }    
    else {
      delayMicroseconds(90/WAIT_MUL_V);
    }
  ...
  long retract_more_cnt = 3000*MUL_V;//13000
  ...
  // if (printingIsActive()) 
  {
    ...
    if (mon_wait_cnt>4000) {
      ...
      if (cur_time>prev_time + 500) {
        ...
        if (b_is_material) {
          b_sub_filament_s_runout = false;
          sub_m_s_hyst_cnt = 0; 
          // SERIAL_ECHOLNPGM("Material is");         
        }
        else {
          sub_m_s_hyst_cnt++;
          // SERIAL_ECHOLNPGM("Material Run out possible"); 
          if (sub_m_s_hyst_cnt>5) {
            b_sub_filament_s_runout = true;
            // SERIAL_ECHOLNPGM("Material Run out issued"); 
          }
        }
...
void PITTA::fila_change(const uint8_t index)
{
  ...
  if (!PITTA::b_retract_already) {
    ...
    retract_ready();
    load_material_from_retract_ready();
  ...
  reset_ext_driver_cnt = 0;
#ifdef STM_FA400_SINGLE_DD_MODE
  b_pitta_single_mode = true;
#else
  b_pitta_single_mode = false;
#endif   
  ...
  // delay(100);
  // while(1) {
  //   b_test_b = chk_material();
  //   if (b_test_b) {
  //     ui.status_printf(0, F("Sensor ON %i"), int(!b_test_b));
  //     SERIAL_ECHOLNPGM("Sensor ON");
  //   }
  //   else {
  //     ui.status_printf(0, F("Sensor OFF "));
  //     SERIAL_ECHOLNPGM("Sensor OFF");
  //   }
  //   delay(10);
  //   test_cnt_dog++;
  //   if (test_cnt_dog>20) {
  ...
  {
    {
      ...
      if (b_pitta_just_enabled||b_pitta_single_mode) {
        while(pitta_get_celsius(0)<185) {
          delay(7);
          pitta_wtcdog_reset();
          pitta_ui_thermal_update();
        }
        SERIAL_ECHOLNPGM("min temp 185 reached");
        delay(1500);
        b_pitta_just_enabled = false; 
        retract_ready();
...
void PITTA::physical_processing() {
  ...
  static long int c_exp_val = 90000;
...
//     }
    ...
    if (jam_expire_cnt > c_exp_val && !b_pitta_jammed)//45000
    {
      b_pitta_jammed = true;
      mat_det_hyst_cnt = 0;
      b_jam_recovered = false;
    ...
    if (loop_watchdog_cnt > 60)
    {
      ...
      // pitta_wtcdog_reset();
      pitta_req_manage_heater_update();
      // ui.status_printf(0, F("F:%i"), int(jam_expire_cnt));
    ...
    if (b_change_done)
    {
      ...
      else
      {
        ...
        while (mot_ext_remain_step > 0 || b_y_spread)
        {
          ...
          currentMillis = millis();
          if ((currentMillis - previousMillis > 50)||(previousMillis-currentMillis>50)) {
            previousMillis = currentMillis;
            pitta_req_manage_heater_update();
          }
         ...
         if (mot_ext_remain_step > init_mot_ext_remain_step - 400*MUL_V)
          {
            ...
            delayMicroseconds(1200/WAIT_MUL_V);//180
            E0_STEP_WRITE(LOW);
            delayMicroseconds(1200/WAIT_MUL_V);//180
          }
          else if (mot_ext_remain_step > init_mot_ext_remain_step - 3000*MUL_V)
          {

            delayMicroseconds(1200/WAIT_MUL_V);//110
            E0_STEP_WRITE(LOW);
            delayMicroseconds(1200/WAIT_MUL_V);//110
          }          
          ...
          {
            {
              delayMicroseconds((400 /* + pitta_extrude_return_spd */)/MUL_V);  // 35
              E0_STEP_WRITE(LOW);
              delayMicroseconds((400 /* + pitta_extrude_return_spd */)/MUL_V);
            }
          }
          else if (mot_ext_remain_step > 2000)
          {
            delayMicroseconds(500/WAIT_MUL_V);//400
            E0_STEP_WRITE(LOW);
            Y_STEP_WRITE(LOW);
            delayMicroseconds(500/WAIT_MUL_V);
          }    
